...
def process_hand(hand, img_flipped):
    ...
    # Since the image is flipped, we swap the hand types
    if img_flipped:
        if handType == "Left":
            handType = "Right"
        else:
...
def video_stream():
    ...
    while True:
        ...
        if hands:
            hand = hands[0]
            lmList, bbox, center, handType, fingers = process_hand(hand, img_flipped=True)
